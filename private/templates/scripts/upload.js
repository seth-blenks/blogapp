...
composition_select_image_button.addEventListener('click', function(){
  image_selection_modal.show();
    fetch('{{url_for("images_p")}}?page=' + 1).then( response => response.text()).then( data => {
      ...
      page_navigation.forEach( item => {
        ...
        item.addEventListener('click',function(){
          ...
          fetch('{{url_for("images_p")}}?page=' + page_number).then( response => response.text()).then( data => {
